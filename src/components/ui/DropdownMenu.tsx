...
function DropdownMenuItem({className, inset, variant = "default", ...props}: React.ComponentProps<typeof DropdownMenuPrimitive.Item> & { inset?: boolean, variant?: "default" | "destructive" }) {
    return (
        <DropdownMenuPrimitive.Item
            ...
            className={cn(
                "focus:bg-secondary focus:text-primary data-[variant=destructive]:text-error data-[variant=destructive]:focus:bg-error/10 dark:data-[variant=destructive]:focus:bg-error/20 data-[variant=destructive]:focus:text-error data-[variant=destructive]:*:[svg]:!text-error [&_svg:not([class*='text-'])]:text-tertiary relative flex cursor-default items-center gap-2 rounded-xs px-2 py-1.5 text-sm outline-hidden select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 data-[inset]:pl-8 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
                className
            )}
            {...props}
        />
...
function DropdownMenuCheckboxItem({className, children, checked, ...props}: React.ComponentProps<typeof DropdownMenuPrimitive.CheckboxItem>) {
    return (
        <DropdownMenuPrimitive.CheckboxItem
            ...
            className={cn(
                "focus:bg-secondary focus:text-primary relative flex cursor-default items-center gap-2 rounded-sm py-1.5 pr-2 pl-8 text-sm outline-hidden select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
                className
            )}
            checked={checked}
            {...props}
...
function DropdownMenuLabel({className, inset, ...props}: React.ComponentProps<typeof DropdownMenuPrimitive.Label> & { inset?: boolean }) {
    return (
        <DropdownMenuPrimitive.Label
            ...
            data-inset={inset}
            className={cn(
                "px-2 py-1.5 text-xs text-tertiary data-[inset]:pl-8",
                className
            )}
            {...props}
        />
...
function DropdownMenuSeparator({className, ...props}: React.ComponentProps<typeof DropdownMenuPrimitive.Separator>) {
    return (
        <DropdownMenuPrimitive.Separator
            ...
            className={cn("bg-main -mx-1 my-1 h-px", className)}
            {...props}
        />
    )
}
...
function DropdownMenuShortcut({className, ...props}: React.ComponentProps<"span">) {
    return (
        <span
            ...
            className={cn(
                "text-tertiary ml-auto text-xs tracking-widest",
                className
            )}
            {...props}
        />
