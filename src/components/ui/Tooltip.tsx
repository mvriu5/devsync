...
const getArrowClasses = (anchor: string) => {
    switch (anchor) {
        ...
        case "tr":
            return {
                arrow: "absolute w-3 h-3 bg-[#f2f2f2] border border-main rotate-45 rounded-br-xs border-t-0 border-l-0",
        ...
        case "br":
            return {
                arrow: "absolute w-2.5 h-2.5 bg-[#f2f2f2] border border-main rounded-tl-xs rotate-45 border-b-0 border-r-0",
        ...
        case "lb":
            return {
                arrow: "absolute w-2 h-2 bg-[#f2f2f2] border border-main rotate-45 rounded-tr-xs border-b-0 border-l-0",
        ...
        default:
            return {
                arrow: "absolute w-2 h-2 bg-[#f2f2f2] border border-main rotate-45 rounded-bl-xs border-t-0 border-r-0",
...
const Tooltip = ({ id, anchor = "rc", width, delay = 1000, icon, message, offset = 8, shortcut, rect, lastTooltipTimestamp, customTooltip, showArrow = true }: TooltipProps & { lastTooltipTimestamp: number | null }) => {
    ...
    return (
        <div
            ...
            id={`tooltip-${id}`}
        >
            <div className={"flex gap-2 items-center text-xs text-black/80"}>
                {icon}
                <span>{message}</span>
            </div>
            {shortcut &&
